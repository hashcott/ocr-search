...
export const documentRouter = router({
  upload: protectedProcedure.input(FileUploadWithOrgSchema).mutation(async ({ input, ctx }) => {
    try {
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const accessibleDocs = await filterDocumentsByPermission(ctx.userId!, documents, 'read');
  ...
  search: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const results = deduplicateByDocument(rawResults).slice(0, input.topK);

      // Get unique document IDs from results
      const documentIds = [
        ...new Set(
          results.map((r: SearchResult) => r.metadata.documentId as string).filter(Boolean)
        ),
      ];
      ...
      const documentsMap = new Map(accessibleDocs.map((doc) => [doc._id.toString(), doc]));
  ...
  share: protectedProcedure
    .input(
      z.object({
        ...
        permissions: z.array(
          z.enum(['manage', 'create', 'read', 'update', 'delete', 'share', 'export'])
        ),
      })
    )
    .mutation(async ({ input, ctx }) => {
      ...
      if (document.userId !== ctx.userId) {
        if (document.organizationId) {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const canShare = ability.can('manage', 'Document', { organizationId: orgId } as any);
          if (!canShare) {
            throw new TRPCError({
              code: 'FORBIDDEN',
      ...
      if (input.shareType === 'user' && input.userIds) {
        ...
        const users = await User.find({
          _id: {
            $in: input.userIds.map((id) => new mongoose.Types.ObjectId(id)),
          },
        });
        ...
        input.userIds.forEach((userId) => {
          if (userId !== document.userId) {
            ...
            const existingIndex = sharedWithUsers.findIndex((s) => s.userId === userId);

            if (existingIndex >= 0) {
              // Update existing share
              sharedWithUsers[existingIndex].permissions = input.permissions;
              sharedWithUsers[existingIndex].sharedBy = ctx.userId!;
              sharedWithUsers[existingIndex].sharedAt = new Date();
            } else {
              // Add new share
        ...
        const sharedWithOrgs = document.sharedWithOrganizations;
        ...
        input.organizationIds.forEach((orgId) => {
          if (orgId !== document.organizationId?.toString()) {
            ...
            const existingIndex = sharedWithOrgs.findIndex(
              (s) => s.organizationId.toString() === orgId
            );

            if (existingIndex >= 0) {
              // Update existing share
              sharedWithOrgs[existingIndex].permissions = input.permissions;
              sharedWithOrgs[existingIndex].sharedBy = ctx.userId!;
              sharedWithOrgs[existingIndex].sharedAt = new Date();
            } else {
              // Add new share
              sharedWithOrgs.push({
  ...
  listUsers: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const users = await User.find(query).select('_id email name').limit(20).lean();
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (document.sharedWithOrganizations && document.sharedWithOrganizations.length > 0) {
        ...
        sharedOrganizations = document.sharedWithOrganizations.map((share) => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const orgIdValue = share.organizationId as any;
          const orgId =
            orgIdValue instanceof mongoose.Types.ObjectId
              ? orgIdValue.toString()
              : String(orgIdValue);
